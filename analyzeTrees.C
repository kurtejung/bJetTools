...
void analyzeTrees(const int startfile=0, const int endfile=1, int isRecopp=1, int ppPbPb=0, int isMuTrig=0, int isMC=1, int doNtuples=1, int doJets=1, int doTracks=1, int updateJEC=1, int cbin=-1, int useGSP=0, int jetTrig=0, bool ExpandedTree=false, bool usePUsub=0, bool useJetTrgAssociation=1, bool doMinBias=0)
{
    ...
    int pthatbin[11] = {15,30,50,80,120,170,220,280,370,460,2000};
    double w = 1.;
    double wght[10]={0.5335, 3.378E-02, 3.778E-03, 4.412E-04, 6.147E-05, 1.018E-05, 2.477E-06, 6.160E-07, 1.088E-07, 2.527E-08};
    ...
    if(ppPbPb){
        ...
        if(doMinBias){
            ...
            infile = "wxie_3rdpPbForest_12-31_rev_dupRemoved.txt"; //"pPbBForestList_Fixed_11-11.txt";
        }
        else if(isMC){
    ...
    if(ppPbPb){
        ...
        else datoutfile = "pPbdata_ppreco_"+jetRecoType+"_jetTrig_etashift_FixTrgCombMethod6_GplusCut_noIPupperCut_"+fileappend+".root";
        string QCDoutfile = "ppMC_ppReco_"+jetRecoType+"_QCDjetTrig_etashift_WeightMethodB_noIPupperCut_"+fileappend+".root";
    ...
    for(int ifile=startfile; ifile<endfile; ifile++){
	    ...
	    //ADDED 2/25!! CROSS CHECK ONLY!
	    //if(isMC) if(pthat<ptboundary[ifile] || pthat>ptboundary[ifile+1]) continue;
