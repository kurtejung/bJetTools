...
void Unfold2(int algo= 3,bool useSpectraFromFile=0, bool useMatrixFromFile=0, int doToy = 0, int isMC = 0,char *spectraFileName = (char*)"pbpb_spectra_ak3PF.root",double recoJetPtCut = 35., int nBayesianIter = 4, int doBjets=1, int doTrigEffCorr=1, int doSkew=0, int doJESshift=0, bool doResSmear=false, int centBin=0) 
{
  bool dopPbOnly = 1;
  bool doPbpOnly = 0;
  ...
  if(dopPbOnly){
    if(useOfficialMC) fileNamePbPb_mc = (char*)"histos/pPbMC_ppReco_akPu3PF_QCDjetTrig_officialMC_etashift_noTrgSelection_MCWeightFinal_WithCentVzWeight_3606.root";
  ...
  if(doPbpOnly){
    if(useOfficialMC)fileNamePbPb_mc = (char*) "histos/pPbMC_ppReco_akPu3PF_QCDjetTrig_REVERSED_officialMC_etashift_noTrgSelection_MCWeightFinal_WithCentVzWeight_3649.root";
  ...
  if (isMC) {
    ...
    if(doPbpOnly) pbpb_Unfo  = new TFile(Form("pPb_Unfo_inCM_v30_officialMC_Reverse_WithResCorr_%s_%s_noGplus_SecondHalfOnly_Converged_usedParameterizedUnfold%d_jtpt%.0f_%s_clo%d_chi%d_v8%s%s%s_eta_%.2fTo%.2f_.root",jetType.c_str(),algoName[algo],doParameterizedMatrix,recoJetPtCut,bJetString.c_str(),cBinLo,cBinHi,skewString.c_str(),resSmearString.c_str(),shiftString.c_str(),etalo,etahi),"RECREATE");
  }
  ...
  if (!useMatrixFromFile) {
    for (Long64_t jentry2=0; jentry2<dataPbPb->tJet->GetEntries();jentry2++) {
      ...
      if ( doPbpOnly && (dataPbPb->jteta > etahi || dataPbPb->jteta < etalo) ) continue;
      ...
      //if (dataPbPb->jtpt/dataPbPb->rawpt > 3.0) continue;  //these next two cuts are negligible for pA
      //if (dataPbPb->rawpt/dataPbPb->jtpt > 3.0) continue;
  ...
  for (int i=0;i<nbins_cent;i++) {
    ...
    if(i==0) uhist[i]->hRecoBinByBin->Draw("same");    
