...
void drawBjetSpectrum(int ppPbPb=1, int pApp=0, int useUnfolded=1, int doInclusiveJet=0, float etalo=-2., float etahi=2., int centLo=0, int centHi=100){
  ...
  else if(pApp) fin2= new TFile(Form("output/ppMC_akPu3PF_bJet_TEST_matchBin0_gsp0_discrSSVHEGT2_eta%.1fTo%.1f.root",ppetalo,ppetahi)); //pp (from MC)
  else  fin2= new TFile("output/NewFormatV5_bFractionMCTemplate_pPbpp1_jetptcut30_SSVHEat2.0FixCL0_bin_0_40_eta_0_2.root"); //pA
  

  if(!useUnfolded) fin = new TFile("output/NewFormatV13_FixedTrgMergingSTAR_ak3PF_fixBin_bFractionMCTemplate_pPbpp1_jetptcut30_SSVHEat2.0FixCL0_bin_0_100_eta_0_3.root");
    ...
    fin = new TFile(Form("pPb_Unfo_inCM_v30_officialMC_ak3PF_akPu3PF_noGplus_FirstHalfOnly_Converged_usedParameterizedUnfold0_jtpt35_bJets_clo%d_chi%d_v8_eta_%.2fTo%.2f_.root",centLo,centHi,etalo,etahi));
    finRev = new TFile(Form("pPb_Unfo_inCM_v30_officialMC_Reverse_WithResCorr_ak3PF_akPu3PF_noGplus_SecondHalfOnly_Converged_usedParameterizedUnfold0_jtpt35_bJets_clo%d_chi%d_v8_eta_%.2fTo%.2f_.root",centLo,centHi,etalo,etahi));
  }
